...
def bench_per_token(shapes):
    ...
    for N, D in shapes:
        ...
        t_f = triton.testing.do_bench(lambda: fused_adaln_fp8(x, scale, shift),
                                      warmup=25, rep=200)
        t_b = triton.testing.do_bench(lambda: baseline_per_token(x, scale, shift),
                                      warmup=25, rep=200)
...
if __name__ == "__main__":
    ...
    shapes = [
        (1024, 3072),     # small (few frames, low res)
        (8192, 3072),
        (65536, 3072),    # big video tile
        (150000, 3072),   # large video
        (1024, 5120),
        (65536, 5120),
    ]
