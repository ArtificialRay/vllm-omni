...
stage1_wan22_i2v() {
    ...
    # Calib-boundary-ratio sweep (per-tensor only) for ablation study.
    # Default per-tensor ckpt above uses 0.5; here we add 0.3 and 0.7.
    run_quant "wan22_i2v_a14b_per_tensor_calib03" "0,1" \
        "${OUTPUT_DIR}/wan22-i2v-a14b-fp8-per-tensor-calib03" \
        "${WAN_PRODUCER}" \
        --model "${WAN_I2V_A14B_MODEL}" \
        --is-i2v --reference-images "${WAN_I2V_REF_IMAGES}" \
        --calib-boundary-ratio 0.3

    run_quant "wan22_i2v_a14b_per_tensor_calib07" "0,1" \
        "${OUTPUT_DIR}/wan22-i2v-a14b-fp8-per-tensor-calib07" \
        "${WAN_PRODUCER}" \
        --model "${WAN_I2V_A14B_MODEL}" \
        --is-i2v --reference-images "${WAN_I2V_REF_IMAGES}" \
        --calib-boundary-ratio 0.7
}
