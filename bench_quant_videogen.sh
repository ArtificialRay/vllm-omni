...
# ============================================================
# Ablation A — Wan2.2 I2V calib-boundary-ratio sweep (per-tensor)
# ============================================================
bench_6_wan22_i2v_per_tensor_calib03() {  # GPU 0
    run_bench "wan22-i2v-a14b-fp8-per-tensor-calib03" "0" \
        --use-offline-quant \
        --model Wan-AI/Wan2.2-I2V-A14B-Diffusers \
        --model-quant-checkpoint "${CKPT_DIR}/wan22-i2v-a14b-fp8-per-tensor-calib03" \
        --task i2v \
        --quantization fp8 \
        --prompts "${PROMPTS[@]}" \
        --negative-prompt "${WAN_I2V_NEGATIVE_PROMPT}" \
...
bench_7_wan22_i2v_per_tensor_calib07() {  # GPU 1
    run_bench "wan22-i2v-a14b-fp8-per-tensor-calib07" "1" \
        --use-offline-quant \
        --model Wan-AI/Wan2.2-I2V-A14B-Diffusers \
        --model-quant-checkpoint "${CKPT_DIR}/wan22-i2v-a14b-fp8-per-tensor-calib07" \
        --task i2v \
        --quantization fp8 \
        --prompts "${PROMPTS[@]}" \
        --negative-prompt "${WAN_I2V_NEGATIVE_PROMPT}" \
...
# ============================================================
# Ablation B — Online vs offline quantization (per-tensor, no checkpoint)
# Online configs omit --use-offline-quant / --model-quant-checkpoint, so the
# Omni runtime calibrates+quantizes at load time. Compare against
# bench_0/2/4 (the per-tensor offline runs) for the same model.
# ============================================================
bench_8_wan22_i2v_online_per_tensor() {   # GPU 0
    run_bench "wan22-i2v-a14b-fp8-online-per-tensor" "0" \
        --model Wan-AI/Wan2.2-I2V-A14B-Diffusers \
        --task i2v \
        --quantization fp8 \
        --prompts "${PROMPTS[@]}" \
...
bench_9_hv15_720p_t2v_online_per_tensor() {   # GPU 1
    run_bench "hv15-720p-t2v-fp8-online-per-tensor" "1" \
        --model hunyuanvideo-community/HunyuanVideo-1.5-Diffusers-720p_t2v \
        --task t2v \
        --quantization fp8 \
        --prompts "${PROMPTS[@]}" \
...
bench_10_vace_14b_r2v_online_per_tensor() {   # GPU 0
    run_bench "wan21-vace-14b-r2v-fp8-online-per-tensor" "0" \
        --model Wan-AI/Wan2.1-VACE-14B-diffusers \
        --task i2v \
        --quantization fp8 \
        --prompts "${PROMPTS[@]}" \
...
# ============================================================
# Ablation C — AdaLN + FP8 fusion ablation (offline per-tensor + fusion ON)
# Compare against bench_0 (Wan2.2) and bench_2 (HV-1.5), which are the same
# offline per-tensor checkpoints with fusion OFF. Wan2.1-VACE doesn't share
# the AdaLN block we fuse, so it's omitted here.
# ============================================================
bench_11_wan22_i2v_per_tensor_adaln_fusion() {  # GPU 1
    run_bench "wan22-i2v-a14b-fp8-per-tensor-adaln-fusion" "1" \
        --use-offline-quant \
...
bench_12_hv15_720p_t2v_per_tensor_adaln_fusion() {  # GPU 0
    run_bench "hv15-720p-t2v-fp8-per-tensor-adaln-fusion" "0" \
        --use-offline-quant \
        --use-fp8-adaln-fusion \
        --model hunyuanvideo-community/HunyuanVideo-1.5-Diffusers-720p_t2v \
        --model-quant-checkpoint "${CKPT_DIR}/hv15-720p-t2v-fp8-per-tensor" \
...
# Ablation A — Wan2.2 calib-boundary-ratio sweep
run_pair "Wan2.2 I2V calib  {0.3 (GPU0) || 0.7 (GPU1)}" \
    bench_6_wan22_i2v_per_tensor_calib03 bench_7_wan22_i2v_per_tensor_calib07

# Ablation B — Online vs offline (3 configs, run as 1 pair + 1 single)
run_pair "Online quant      {Wan2.2 I2V (GPU0) || HV-1.5 T2V (GPU1)}" \
    bench_8_wan22_i2v_online_per_tensor bench_9_hv15_720p_t2v_online_per_tensor

# Ablation C — AdaLN+FP8 fusion paired with the remaining VACE online run
run_pair "Mixed last pair   {VACE online (GPU0) || Wan2.2 adaln-fusion (GPU1)}" \
    bench_10_vace_14b_r2v_online_per_tensor bench_11_wan22_i2v_per_tensor_adaln_fusion

echo "[$(ts)] Final single: HV-1.5 adaln-fusion (GPU 0)"
bench_12_hv15_720p_t2v_per_tensor_adaln_fusion || \
    echo "[$(ts)] HV-1.5 adaln-fusion FAILED" >&2
