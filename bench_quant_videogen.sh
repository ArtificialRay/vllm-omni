...
# ============================================================
# Transfer results + close connection
# ============================================================
# Set LOCAL_DEST to enable, e.g.:
#   LOCAL_DEST=me@laptop:/Users/me/bench_results        (rsync over SSH)
#   LOCAL_DEST=/mnt/shared/bench_results                (local mount / NFS)
# When unset, the transfer step is skipped and ${OUTPUT_ROOT} remains in place.
# Optional: rsync flags can be customized via RSYNC_OPTS (default -avz --progress).
transfer_results() {
    if [[ -z "${LOCAL_DEST:-}" ]]; then
        echo "[$(ts)] LOCAL_DEST not set — leaving results in ${OUTPUT_ROOT}."
        echo "         Re-run with LOCAL_DEST=user@host:/path to auto-transfer."
        return 0
    fi

    local rsync_opts="${RSYNC_OPTS:--avz --progress}"
    echo "[$(ts)] Transferring ${OUTPUT_ROOT} → ${LOCAL_DEST}"
    echo "        rsync ${rsync_opts} ${OUTPUT_ROOT}/ ${LOCAL_DEST}/"
    # Trailing slashes ensure we sync the *contents* of OUTPUT_ROOT into
    # LOCAL_DEST (vs. nesting OUTPUT_ROOT inside LOCAL_DEST).
    if rsync ${rsync_opts} "${OUTPUT_ROOT}/" "${LOCAL_DEST}/"; then
        echo "[$(ts)] Transfer complete."
        return 0
    else
        local rc=$?
        echo "[$(ts)] Transfer FAILED (rc=${rc}). Results remain in ${OUTPUT_ROOT}." >&2
        return ${rc}
    fi
}

transfer_results || exit $?

# Returning from main exits the shell; if this script was invoked through
# `ssh remote 'bash bench_quant_videogen.sh'` (or piped over SSH), the SSH
# session closes when the script returns. Inside tmux/screen, the pane stays
# alive until the user detaches/exits — that's intentional, not a bug.
echo "[$(ts)] All done. Exiting."
exit 0
